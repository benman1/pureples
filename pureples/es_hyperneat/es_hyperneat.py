...
class ESNetwork:
    ...
    def create_phenotype_network(self, filename=None):
        ...
        # For every coordinate, check the connections and create a node with
        # corresponding connections if appropriate.
        ...
        # Combine the indices with the connections/links forming node_evals
        # used by the RecurrentNetwork.
        ...
        return neat.nn.RecurrentNetwork(
            list(input_nodes), list(output_nodes), node_evals)
    ...
    # Determines which connections to express - high variance = more
    # connetions.
    def pruning_extraction(self, coord, p, outgoing):
        for c in p.cs:
            ...
            con = None
            if max(min(d_top, d_bottom), min(d_left, d_right)
                   ) > self.band_threshold:
                if outgoing:
            ...
            if con is not None:
                # Nodes will only connect upwards. If connections to same layer
                # is wanted, change to con.y1 <= con.y2.
                if not c.w == 0.0 and con.y1 < con.y2 and not (
                        con.x1 == con.x2 and con.y1 == con.y2):
    ...
    # Clean a net for dangling connections by intersecting paths from input
    # nodes with paths to output.
    def clean_net(self, connections):
        ...
        for c in connections:
            # Only include connection if both source and target node resides in
            # the real path from input to output
...
# Class representing an area in the quadtree defined by a center
# coordinate and the distance to the edges of the area.
class QuadPoint:
...
# Class representing a connection from one point to another with a certain
# weight.
class Connection:
...
# From a given point, query the cppn for weights to all other points. This
# can be visualized as a connectivity pattern.
